 ...
 get_header(); ?>
 	...
 	<div class="main-container nobg container-video" id="presentation"></div>
        ...
        <div id="code_promo" class="main-container" style="background-color: rgba(247,247,247,0.8)">
                	...
                	<section>
                        ...
                        <table>
                            ...
                            <tbody>
                             ...
                             <?php
					            $args = array( 'post_type' => 'product', 'posts_per_page' => -1, 'numberposts' => -1, 'stock' => 1, 'orderby' =>'meta_value_num', 'meta_key'  => '_price', 'order' => 'ASC' ); 
					            $loop = new WP_Query( $args );
					            $cpt=1;
					            while ( $loop->have_posts() ) : $loop->the_post(); global $product;
					        ...
					        ?>
		                       <tr class="cuv_num" value="<?php the_ID(); ?>">
									...
									<td class="chiffre"><input type="text" name="prix_<?php echo($cpt); ?>" id="prix_<?php echo($cpt); ?>" class="hide" readonly value="<?php echo $product->get_price(); ?>" />
									<?php if( $product->get_sale_price() > 0 ) { ?>
											<input type="text" name="prix_regular_aff_<?php echo($cpt); ?>" id="prix_regular_aff_<?php echo($cpt); ?>" style="text-decoration: line-through;" class="regular-price-sale" readonly value="<?php echo $product->get_regular_price(); ?> €" />
											<input type="text" name="prix_aff_<?php echo($cpt); ?>" id="prix_aff_<?php echo($cpt); ?>" readonly value="<?php echo $product->get_price(); ?> €" /><?php } else { ?>
											<input type="text" name="prix_aff_<?php echo($cpt); ?>" id="prix_aff_<?php echo($cpt); ?>" readonly value="<?php echo $product->get_price(); ?> €" />
									<?php }?>
									</td>
         ...
         <script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBYiX7Bwk_sJbtCys7GN9DIAyFmndWrlvg&sensor=false"></script>
